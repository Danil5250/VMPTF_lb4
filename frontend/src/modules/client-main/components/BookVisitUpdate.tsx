...
import {addFullVisit, generateVisitRecord, getVisitInfo, updateVisit} from "../../visits/api/visitPageService.ts";
...
import {getAutorepairServicesById} from "../../autorepairs/api/autorepairPageService.ts";
...
interface BookingModalUpdateProps {
    visitId: number;
    onClose: () => void;
}
...
const BookingModalUpdate = ({ visitId, onClose }: BookingModalUpdateProps) => {
    ...
    const [autorepairId, setAutorepairId] = useState(null)
    const [autorepairName, setAutorepairName] = useState(null)
    ...
    useEffect(() => {
        const fetchVisitData = async () => {
            try {
                const result = await getVisitInfo(visitId);
                ...
                setSelectedDate(data.visit.date_time ? new Date(data.visit.date_time) : null);
                setAlternativeDate(
                    data.visit.alternative_date_time ? new Date(data.visit.alternative_date_time) : null
                );

                setAdditionalNote(data.visit.note || "");

                if (data.selectedServices.length > 0) {
                    setBookingType("select");
                } else {
                    setBookingType("describe");
                    setProblemDescription(data.problemDescription.problem_description || "");
                }

                setSelectedServiceIds(
                    data.selectedServices.map((s: any) => s.autorepair_service_id)
                );
                ...
                setSelectedCarId(data.visit.car_id)

                setServices(data.allServices);

                setAutorepairId(data.autorepair.autorepair_id);
                setAutorepairName(data.autorepair.name);
    ...
    useEffect(() => {
        if(selectedCarId && userCars){

            const selectedCar = userCars.find((value) => value.car_id == selectedCarId);
            ...
            if(bookingType === 'select') {
                ...
                try {
                    ...
                    await updateVisit(visitId, {
                        ...
                        visit_selectedAlternativeDate: finalAltDate ? finalAltDate.toISOString() : null,
                        visit_note: additionalNote || null,
                        // visit_paymentWay: data.visit.payment_way,
                        // visit_paymentStatus: null,
                        // visit_isCompleted: null,
                        visit_carid: String(carData.car_id),
    ...
    const generateVisitReport = async() => {
        const pdfBlob = await generateVisitRecord({ visitId, clientId: user.id, carId: carData.car_id });

        const pdfUrl = window.URL.createObjectURL(new Blob([pdfBlob], { type: "application/pdf" }));

        Swal.fire({
            title: "<strong style='font-size:28px;color:#1e293b'>Запис успішно оновлено!</strong>",
            html: `
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    ...
                    {step === 3 && (
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-600 mb-6">
                                Оберіть один із ваших збережених автомобілів або додайте новий
                            </p>
                                    ...
                                    >
                                        <input
                                            ...
                                            onChange={(e) => {
                                                ...
                                                const selectedCar = userCars.find(car => car.car_id === carId);
                                                console.log(selectedCar)
                                                if (selectedCar) {
                                                    setCarData({
                                                        car_id: selectedCar.car_id,
                    ...
                    {step === 6 && (
                        <div className="text-center py-8">
                            ...
                            <h2 className="text-2xl font-semibold text-slate-800 mb-2">Запис оновлено!</h2>
